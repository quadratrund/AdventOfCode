...
while (currentSteps.every(step => step.x !== grid[0].length - 1 || step.y !== grid.length - 1)) {
  ...
  const nextSteps = allowedDirections
    ...
    .filter(nextStep => {
      ...
      for (let i = nextStep.straightCounter; i < 4 && distances[i] == null; i++) {
